...
export default class EventListPresenter {
  ...
  init() {
    ...
    render(new FormEventView({eventData: this.boardEvents[FORM_EVENT_INDEX],
      ...
      offersData: this.boardOffers[indexOffers]}), this.eventListComponent.getElement());
    //ОТРИСОВКА ТОЧЕК
    for (let i = 1; i < this.boardEvents.length; i++) {
      indexDestination = this.boardDestinations.findIndex((destination) => destination.id === this.boardEvents[i].destination);
      indexOffers = this.boardOffers.findIndex((offer) => offer.type === this.boardEvents[i].type);
